...
inline r_t expectimax_afterstate(u64 afterstate, u8 depth, r_t max_prob, u64 &evals, const NTuple &tuples);

inline Eval expectimax_state(const u64 state, const u8 depth, const r_t max_prob, u64 &evals, const NTuple &tuples) {
    Eval best = Eval::None();
...
inline r_t expectimax_afterstate(const u64 afterstate, u8 depth, const r_t max_prob, u64 &evals, const NTuple &tuples) {
    if (evals == 0) { return 0; }
...
inline Eval expectimax_limited_depth_prob(const u64 board, const u8 depth, const r_t prob, const NTuple &tuples) {
...
inline Eval expectimax_limited_evals(const u64 board, u64 evals, const r_t threshold, const NTuple &tuples) {
...
inline u64 downgraded(u64 board, const u8 threshold = 15) {
    u32 used = 0;
    u8 highest = 0;
    for (u8 i = 0; i < 16; ++i) {
...
inline u64 upgraded_old(const u64 board, const u8 threshold) {
    array<u8, 16> counts{};
    counts.fill(0);
    for (u8 i = 0; i < 16; ++i) {
...
inline u64 upgraded(const u64 board) {
    array<u8, 16> counts{};
    for (u8 i = 0; i < 16; ++i) {
        ++counts[get_cell(board, i)];
    }
