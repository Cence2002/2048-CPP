...
inline std::vector<Game_stat> run_algorithm_episodes(u32 games, u8 threads, Dir (*algorithm)(const b_t)) {
    ...
    std::cout << indent << "S per game:    \t" << (elapsed / 1e6) / r_t(testing_stats.game_counter) << std::endl;
    std::cout << indent << "Moves per uS:  \t" << r_t(testing_stats.moves_counter) / elapsed << std::endl;
