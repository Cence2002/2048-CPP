...
struct Bruteforce {
    ...
    float eval_state(u64 state) {
        ...
        for (const Dir d: Dirs) {
            ...
            float curr_eval = eval_afterstate(afterstate, 1);
            switch (metric) {
                case 0:
                    curr_eval += 1;
                    break;
                case 1:
                    curr_eval += float(get_reward<3>(state, d));
                    break;
                default:
    ...
    float eval_afterstate(u64 afterstate, u8 depth) {
        ...
        for (u8 i = 0; i < 9; ++i) {
            if (empty & mask) {
                for (const auto &[shift, prob]: {make_pair(0, 0.9), make_pair(1, 0.1)}) {
                    float curr_eval = eval_afterstate(afterstate | (mask << shift), depth - 1);
