...
template<board_concept board_type=b_t>
s_t play_random_game() {
    ...
    while (true) {
        vector<Dir> dirs;
        if (board.slid(Left) != board) { dirs.push_back(Left); }
        if (board.slid(Right) != board) { dirs.push_back(Right); }
        if (board.slid(Up) != board) { dirs.push_back(Up); }
        if (board.slid(Down) != board) { dirs.push_back(Down); }
        if (dirs.empty()) { break; }
        const Dir d = dirs[random(dirs.size())];
...
template<board_concept board_type=b_t>
r_t performance_test(const u32 count) {
    board_type::init_tables();
    auto start = time_now();
    r_t sum = 0;
    for (u32 i = 0; i < count; ++i) {
        sum += r_t(play_random_game<board_type>());
    }
    cout << "Average score: " << sum / r_t(count) << endl;
